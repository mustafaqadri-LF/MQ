...
const steps = [
  ...
  },
  {
    ...
    icon: <Send className="w-12 h-12 text-emerald-400" />,
...
export default function AppProvisioning() {
  ...
  return (
    <div className="p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-700 p-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">App Registration & Credential Flow</h1>
            <p className="text-sm text-slate-400 mt-1">Management Plane to Traffic Plane Synchronization</p>
          </div>
          <div className="flex gap-3">
            ...
            {isPlaying && currentStep <= steps.length && (
              <button onClick={nextStep} className="flex items-center gap-2 bg-white hover:bg-slate-100 text-slate-900 px-5 py-2.5 rounded-lg font-medium transition-colors">
            ...
            {currentStep > 0 && (
              <button onClick={resetSimulation} className="flex items-center gap-2 border border-slate-600 hover:bg-slate-800 text-slate-300 px-5 py-2.5 rounded-lg font-medium transition-colors">
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-700 p-8 min-h-[400px] flex flex-col relative overflow-hidden">
          {currentStep === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
              <CloudCog className="w-20 h-20 text-slate-600 mb-2" />
              <h2 className="text-xl font-semibold text-slate-300">Ready to Initiate</h2>
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-center mb-10 relative">
                <div className="absolute left-0 top-1/2 w-full h-0.5 bg-slate-700 -z-10 transform -translate-y-1/2"></div>
                <div
                ...
                ></div>
                {steps.map((step) => (
                  <div key={step.id} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 border-2 ${currentStep >= step.id ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-slate-800 border-slate-600 text-slate-500'}`}>
                    {step.id}
                  </div>
                ))}
              </div>
              ...
              <div className="flex-1 flex gap-8 items-stretch">
                <div className="w-1/3 bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center justify-center p-6 text-center">
                  <div className="bg-slate-900 p-4 rounded-full mb-4 border border-slate-700">
                    {activeStepData.icon}
                  </div>
                  <span className="text-xs font-bold uppercase tracking-wider text-blue-400 bg-blue-900/30 px-3 py-1 rounded-full mb-3">{activeStepData.plane}</span>
                  <h3 className="text-lg font-bold text-white">{activeStepData.title}</h3>
                </div>

                <div className="w-2/3 flex flex-col justify-center space-y-6">
                  <p className="text-lg text-slate-300 leading-relaxed">{activeStepData.description}</p>
                  <div className="grid grid-cols-1 gap-4">
                    {Object.entries(activeStepData.details).map(([key, value]) => (
                      <div key={key} className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex items-start gap-4">
                        <div className="w-24 shrink-0 font-semibold text-slate-200 text-sm">{key}</div>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-700 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      {!aiExplanations[activeStepData.id] && (
                        <button
                          ...
                          className="text-xs bg-indigo-900/40 hover:bg-indigo-900/60 text-indigo-300 font-medium px-3 py-1.5 rounded-md transition-colors flex items-center gap-1.5 disabled:opacity-50"
                        >
                    ...
                    {aiExplanations[activeStepData.id] && (
                      <div className="bg-indigo-900/20 border border-indigo-800/50 rounded-lg p-4 text-sm text-indigo-200 leading-relaxed">
                    ...
                    <div className="space-y-3">
                      {chatAnswers[activeStepData.id] && (
                        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 text-sm space-y-2">
                          <div className="text-slate-400 font-medium pb-2 border-b border-slate-700">
                            <span className="text-slate-500 mr-2">Q:</span>{chatAnswers[activeStepData.id].question}
                          </div>
                          <div className="text-slate-200 leading-relaxed pt-1">
                            <span className="text-indigo-400 mr-2 font-medium">A:</span>{chatAnswers[activeStepData.id].answer}
                          </div>
                        </div>
                      )}
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 border border-slate-700 bg-slate-800 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        />
                        <button
                          onClick={() => handleAskQuestion(activeStepData)}
                          disabled={isGeneratingAnswer || !chatInput.trim()}
                          className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800 disabled:text-indigo-400 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
                        >
